...
char cmdlne[1024];
char progid[80] = { "$Id: epopsound.c,v 1.02 2015/11/09 22:00:00 KKrieger Exp $" };
char progname[256];
...
int main(int argc, char *argv[])
{
	...
	int mpinc_7 = 2400;
/*	int dmpinc_7 = 2400;
	int nmpinc_7 = 2400;  These are not used anywhere else here */
...
/*	int scnsc = 1;	
	int scnus = 0;
	int skip;  These variables are not applicable in ePOPsound, or never used after */
	int cnt = 0;
	...
	intsc = 1;
	intus = 0;		/* picked to be small enough to get 1
	...
	nrang = 75;
	rsep = 45;
	txpl = 300;     /* Pointless here as computed below based on rsep */
	stfrq = nfrq;
	frang = nfrang;
	SiteStart();
