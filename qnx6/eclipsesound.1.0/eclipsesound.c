/* eclipsesound.c
 ...
 Modified from interleavesound.c by K.T. Sterne
...
int main(int argc,char *argv[]) {
  ...
  int nintgs=11;      /* number of integration periods per scan; SGS 1-min  */
  unsigned char hlp=0;
  ...
  float snd_time, snd_intt, time_needed=0.5;
  ...
  if (snd_dat != NULL) {
    ...
    sprintf(logtxt,"Sounder File: %s read",snd_filename);
  ...
  sprintf(progname,"eclipsesound (fast)");
  for (i=0; i<nintgs; i++){
